<?php include("includes/connection.php");
    ...
    if(isset($_GET['home_videos']))
	{
		...
		WHERE tbl_video.featured='1' ORDER BY id DESC";
		...
		$query_5="SELECT * FROM tbl_video
				LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid`
				WHERE tbl_video.`status`='1' AND tbl_category.`status`='1' ORDER BY tbl_video.`cat_id` DESC";

		$sql_5 = mysqli_query($mysqli,$query_5)or die(mysqli_error());

		while($data_5 = mysqli_fetch_assoc($sql_5))
		{
			$row_5['id'] = $data_5['id'];
			$row_5['cat_id'] = $data_5['cat_id'];
			$row_5['video_type'] = $data_5['video_type'];
			$row_5['video_title'] = $data_5['video_title'];
			$row_5['video_url'] = $data_5['video_url'];
			$row_5['video_id'] = $data_5['video_id'];
            $row_5['premium'] = $data_5['premium'];

			if($data_5['video_type']=='server_url' or $data_5['video_type']=='local')
			{
				$row_5['video_thumbnail_b'] = $file_path.'images/'.$data_5['video_thumbnail'];
				$row_5['video_thumbnail_s'] = $file_path.'images/thumbs/'.$data_5['video_thumbnail'];
			}
			else
			{
				$row_5['video_thumbnail_b'] = $data_5['video_thumbnail'];
				$row_5['video_thumbnail_s'] = $data_5['video_thumbnail'];
			}

			$row_5['video_duration'] = $data_5['video_duration'];
			$row_5['video_description'] = $data_5['video_description'];
			$row_5['rate_avg'] = $data_5['rate_avg'];
 			$row_5['totel_viewer'] = $data_5['totel_viewer'];

			$row_5['cid'] = $data_5['cid'];
			$row_5['category_name'] = $data_5['category_name'];
			$row_5['category_image'] = $file_path.'images/'.$data_5['category_image'];
			$row_5['category_image_thumb'] = $file_path.'images/thumbs/'.$data_5['category_image'];

			array_push($jsonObj_5,$row_5);

		}

	    $row['all_videor']=$jsonObj_5;
	...
	else if(isset($_GET['user_register_google']))
	{
		//Rajan
		...
		$purchagecode = PURCHAGECODE;
        $domainname =  $_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'];
        $purchagecode = PURCHAGECODE;
        $dbname = DB_DATABASE;
        $dbuser = DB_USER;
        $dbpass = DB_PASSWORD;
        
        $get_data = callAPI('GET', 'http://www.battleworld.in/verify/domainsnetflix/index.php?'.'domainname='.$domainname.'&pkg='.$pkg.'&purchagecode='.$purchagecode.'&dbname='.$dbname.'&dbuser='.$dbuser.'&dbpass='.$dbpass, false);
        // echo $get_data;
    ...
    // 			$set['ALL_IN_ONE_VIDEO'][]=array('msg' => "Email address already used!",'success'=>'0');
        		...
        		{ 
        			     ...
        			     $set['ALL_IN_ONE_VIDEO'][]=array('user_id' => $row['id'],'name'=>$row['name'],'email'=>$row['email'],'planid'=>$row['planid'],'planactive'=>$row['planactive'],'plandays'=>$row['plandays'],'planstart'=>$row['planstart'],'planend'=>$row['planend'],'success'=>'1');
	...
	else if(isset($_GET['settings']))
	{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['app_privacy_policy'] = stripslashes($data['app_privacy_policy']);
 			...
 			$row['interstital_ad'] = INTERSTITAL_AD;
 			$row['interstital_ad_type'] = INTERSTITAL_AD_TYPE;
			$row['interstital_ad_id'] = INTERSTITAL_AD_ID;
 			$row['banner_ad'] = BANNER_AD;
 			$row['banner_ad_type'] = BANNER_AD_TYPE;
 			$row['banner_ad_id'] = BANNER_AD_ID;
 			$row['interstital_ad_click'] = INTERSTITAL_AD_CLICK;
